...
class Assets {
	...
	public function load_admin_scripts() {
		...
		if ( $typenow === 'igmap' && ( $pagenow === 'post.php' || $pagenow === 'post-new.php' ) ) {
			...
			$this->options['messages'] = [
				...
				'maxInputError' => __( 'Attention! Your map has too many entries for your server to handle when saving it. <a href="https://interactivegeomaps.com/docs/max-input-vars-issue" target="_blank">Read more about this issue here</a>.', 'interactive-geo-maps' ),
	...
	public function load_map_scripts( $localize = false ) {
		...
		$version = 'version/4.9.10';

		if( isset( $options['amcharts_version'] ) && trim($options['amcharts_version']) !== '' ){
			$version = 'version/' . $options['amcharts_version'];
		}

		$url_path = 'https://www.amcharts.com/lib/';
		...
		// if we won't be using async loading
			...
			$this->modify_script_tag( $this->core->name . '_amcharts_core' );
		...
		wp_register_script(
			...
			plugins_url( 'assets/maps-service/assets/js/app.min.js', $this->core->file_path ),
			$deps,
			$this->core->version,
			true
		);
