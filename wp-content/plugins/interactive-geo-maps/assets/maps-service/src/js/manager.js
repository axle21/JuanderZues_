...
iMapsManager.handleZoom = function (id) {
	...
	if (typeof data.viewport !== "undefined" && data.viewport.offset) {
		...
		if (defaultOffset) {
			iMapsManager.latlongOffsetFix(data.map, map, defaultZoom);
		}

	}

	// manual fixes for NZ and RU
...
iMapsManager.latlongOffsetFix = function (mapSelected, mapObj, defaultZoom) {
	if (mapSelected === 'russiaLow' || mapSelected === 'russiaHigh' || mapSelected === 'russiaCrimeaHigh' || mapSelected === 'russiaCrimeaLow') {
		mapObj.deltaLongitude = -100;

		if (defaultZoom) {
			mapObj.homeZoomLevel = 2;
		}
...
iMapsManager.pushLabelSeries = function (id, data) {
	...
	if (Array.isArray(data.labels) && data.labels.length) {
		...
		labelSeriesTemplate.propertyFields.verticalCenter = "verticalCenter";
		labelSeriesTemplate.propertyFields.horizontalCenter = "horizontalCenter";
...
iMapsManager.setupHoverEvents = function (id, ev) {
	...
	if (im.bool(dataContext.triggerClickOnHover)) {
		iMapsManager.select(id, dataContext.id);
	}
};
